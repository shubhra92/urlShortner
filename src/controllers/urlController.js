...
const createUrl= async function(req,res){
    ...
    if(typeof longUrl === 'string')return res.status(400).send({status:false,message:"Plz provide the url"})
    ...
    if(checkUrl){
      checkUrl=JSON.parse(checkUrl)
       return res.status(200).send({status:true,message:"this response came from redis",data:checkUrl})}
    ...
    if(checkurl){ await SET_EX(`${checkurl.longUrl}`,  DEFAULT_EXPIRATION, JSON.stringify(checkurl))
               await SET_EX(`${checkurl.urlCode}`,  DEFAULT_EXPIRATION, JSON.stringify(checkurl))
